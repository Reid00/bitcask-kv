...
type (
	...
	sklNode struct {
		member   string      // 节点成员
		score    float64     // 节点得分
		backward *sklNode    // 下一层 链表节点
		level    []*sklLevel // 索引层级，三级索引 需要在三 二 一 层都插入
	}
...
func (skl *skipList) sklInsert(score float64, member string) *sklNode {
	...
	p = newsklNode(level, score, member)
...
func (skl *skipList) sklDelete(score float64, member string) {
	...
	for i := skl.level - 1; i >= 0; i-- {
		for p.level[i].forward != nil && (p.level[i].forward.score < score ||
			(p.level[i].forward.score == score && p.level[i].forward.member < member)) {
			p = p.level[i].forward
		}
		update[i] = p
