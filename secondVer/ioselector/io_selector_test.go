...
func testNewIOSelector(t *testing.T, ioType uint8) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			absPath, err := filepath.Abs(filepath.Join("c:/tmp", tt.args.fname))
			assert.Nil(t, err)
...
func testIOSelectorWrite(t *testing.T, ioType uint8) {
	absPath, err := filepath.Abs(filepath.Join("c:/tmp", "00000001.vlog"))
	assert.Nil(t, err)
	...
	tests := []struct {
		...
		{
			"many-byte", fields{selector: selector}, args{b: []byte("Reid'sdb"), offset: 0}, 8, false,
		},
		{ // 用1024个字节填充123前面的宽度
		...
		{
			"EOF-error", fields{selector: selector}, args{b: []byte("Reid's db"), offset: -1}, 0, true,
		},
	}
...
func testIOSelectorRead(t *testing.T, ioType uint8) {
	absPath, err := filepath.Abs(filepath.Join("c:/tmp", "00000001.wal"))
	assert.Nil(t, err)
	...
	tests := []struct {
		...
		{
			"many-byte", fields{selector: selector}, args{b: make([]byte, 7), offset: offsets[2]}, 7, false,
		},
		{
...
func testIOSelectorSync(t *testing.T, ioType uint8) {
	sync := func(id int, fsize int64) {
		absPath, err := filepath.Abs(filepath.Join("c:/tmp", fmt.Sprintf("0000000%d.wal", id)))
		assert.Nil(t, err)
...
func testIOSelectorDelete(t *testing.T, ioType uint8) {
	...
	for i, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			absPath, err := filepath.Abs(filepath.Join("c:/tmp", fmt.Sprintf("0000000%d.wal", i)))
			assert.Nil(t, err)
			var selector IOSelector
			if ioType == 0 {
			...
			if ioType == 1 {
				selector, err = NewMMapSelector(absPath, int64((i+1)*100))
			}
